...
void Double_sentinel_list_tester<Type>::process() {
	if ( command == "new" ) {
		...
		*new_object = *(object);
		...
		tester.run();
    //Ensures that the move ctr works and anything thats moved is safe to delete afterwards. Used the same way as assign
	} else if (command == "move"){
		Double_sentinel_list<Type> *temp = new Double_sentinel_list<Type>(*object);

		Double_sentinel_list<Type> *new_object = new Double_sentinel_list<Type>(std::move(*object));

		delete object;
		object = new Double_sentinel_list<Type>(std::move(*temp));
		delete temp;
